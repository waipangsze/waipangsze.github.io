...
for gfsfile in $(seq 0 3 3); do # $(seq 0 3 216), Forecast time = 12, 24, 36, 48, 60, 72, 84, 96, 108, 130, 142, 154
  ...
  # wget -c https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.20250526/00/atmos/gfs.t00z.pgrb2.0p25.f000 
  wget -c https://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${yyyy}${mm}${dd}/${hh}/atmos/gfs.t${hh}z.pgrb2.0p25.f${tmp_f3d} -O ${tmp_gfs_fname} 
...
for gfsfile in $(seq 0 3 12); do # Forecast time = 12, 24, 36, 48, 60, 72, 84, 96, 108, 130, 142, 154
	...
	echo "downloading ..."
	wget -c --no-check-certificate -N --load-cookies auth.rda_ucar_edu https://rda.ucar.edu/data/ds084.1/${tmp}
done
```
