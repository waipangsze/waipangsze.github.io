...
- These flags tell the scheduler exactly how many nodes, how many tasks (usually one MPI rank/process per task), and how many cores (CPUs) per task you want.

### `--exclusive`

- Add `--exclusive` to your sbatch script
  - e.g. You want to occupy 4 entire nodes exclusively (no other users' jobs can run on them), but still use only 2 cores per node for your tasks.

{% note primary %}
You want to occupy 4 entire nodes exclusively (no other users' jobs can run on them), but still use only 2 cores per node for your tasks.
{% endnote %}
...
#SBATCH --cpus-per-task=1            # 1 core per MPI task → 2 cores/node
#SBATCH --exclusive                  # Nodes are occupied by you only (no sharing)
#SBATCH --time=00:10:00              # 10 minutes should be enough for test
#SBATCH --output=mpi_test_%j.out     # Output file
#SBATCH --error=mpi_test_%j.err      # Error file

# Print some useful Slurm environment info
...
mpirun -np $SLURM_NTASKS python3 mpi_hello.py
```
...
# Get the hostname (node name)
hostname = socket.gethostname()

print(f"Hello from rank {rank} out of {size} processes | Running on node: {hostname} | PID: {MPI.Get_processor_name() if hasattr(MPI,'Get_processor_name') else 'N/A'}")

# Small delay so output doesn't interleave too much
time.sleep(0.1)

# Rank 0 prints a summary at the end
if rank == 0:
    print(f"\n=== MPI Test Completed ===")
    print(f"Total MPI processes: {size}")
    print(f"Nodes requested: 4")
    print(f"Tasks per node: 2")
    print(f"Each using 1 core → 2 cores per node")
```
