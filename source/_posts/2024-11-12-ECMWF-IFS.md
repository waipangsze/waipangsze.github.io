...
- ['Error in interpolation of st_fg to MPAS grid' with variable-resolution mesh simulation](https://forum.mmm.ucar.edu/threads/error-in-interpolation-of-st_fg-to-mpas-grid-with-variable-resolution-mesh-simulation.9387/)
- It generally indicates that there are grid cells that have a soil temperature value less than or equal to zero.
...
and MPASv7.3 runs (./atmosphere_model) with IFS successfully as well (**both with Vtable.txt above or Vtable.ECMWF from WPSv4.6**). 
