...
![](https://i.imgur.com/MEQ6TA1.png){width=400}
...
- [Running WRF/WPS with ECMWF IFS Open Data (soil layer issue and fix)​](https://forum.mmm.ucar.edu/threads/help-needed-%E2%80%93-wrf-setup-using-ecmwf-ifs-data-ungrib-failure.22072/post-60271)
...
- <https://www.ecmwf.int/sites/default/files/elibrary/2023/81353-migration-grib1-grib2-preparing-ecmwf-model-output-future.pdf>
...
echo "Today is: $yyyy-$mm-$dd $hh:00"
...
for ifsfile in $(seq 0 3 6); do # Forecast time = 12, 24, 36, 48, 60, 72, 84, 96, 108, 130, 142, 154
	...
	echo "Now, ${tmp_ifs_fname} ... "
	echo "Downloading ... ${tmp_ifs_fname}"
	wget --no-check-certificate -c https://data.ecmwf.int/forecasts/${yyyy}${mm}${dd}/${hh}z/ifs/0p25/oper/${tmp_ifs_fname}
	res=$(( ${count_cpu_core} % ${limit_cpu_core} ))
	if [[ ${res} == 0 ]]; then
		echo "please wait ${tmp_ifs_fname}, ${count_cpu_core}, ${res}."
		wait
...
for file in *h-oper-fc.grib2; do
    ...
    if [[ $file =~ (.*-)([0-9]+)(h-.*) ]]; then
        ...
        # Format the hour to be 3 digits with leading zeros
        printf -v padded_hour "%03d" $(( 10#${hour} ))

        # Rename the file and softlink
        ln -sf "$file" "${prefix}${padded_hour}${suffix}"
    fi
done
```
{% endfold %}
